...
use Fabricate\Http\Client\Events\ConnectionFailed;
use Fabricate\Http\Client\Events\RequestSending;
use Fabricate\Http\Client\Events\ResponseReceived;
use Fabricate\Http\Client\Promises\FluentPromise;
...
use function Fabricate\NutsAndBolts\Helpers\retry;
...
class PendingRequest
{
     ...
     *
     * @var \Fabricate\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \Fabricate\NutsAndBolts\Collection
     */
    protected $beforeSendingCallbacks;

    /**
     * The callbacks that should execute after the Fabricate Response is built.
     *
     * @var \Fabricate\NutsAndBolts\Collection<int, (callable(\Fabricate\Http\Client\Response): \Fabricate\Http\Client\Response|null)>
     */
     ...
     *
     * @var \Fabricate\NutsAndBolts\Collection|null
     */
     ...
     *
     * @var \Fabricate\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  \Fabricate\Http\Client\Factory|null  $factory
     ...
     *
     * @param  (callable(\Fabricate\Http\Client\Response): \Fabricate\Http\Client\Response|null)  $callback
     * @return $this
     */
     ...
     * @return \Fabricate\Http\Client\Response|\GuzzleHttp\Promise\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \Fabricate\Http\Client\Response : \GuzzleHttp\Promise\PromiseInterface)
     *
     * @throws \Fabricate\Http\Client\ConnectionException
     */
     ...
     *
     * @param  string  $url
     * @param  array|\JsonSerializable|\Fabricate\NutsAndBolts\Contracts\Arrayable  $data
     * @return \Fabricate\Http\Client\Response|\GuzzleHttp\Promise\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \Fabricate\Http\Client\Response : \GuzzleHttp\Promise\PromiseInterface)
     *
     ...
     *
     * @param  (callable(\Fabricate\Http\Client\Pool): mixed)  $callback
     * @param  non-negative-int|null  $concurrency
     * @return array<array-key, \Fabricate\Http\Client\Response|\Throwable>
     */
     ...
     *
     * @param  callable  $callback
     * @return \Fabricate\Http\Client\Batch
     */
     ...
     * @param  string  $url
     * @param  array  $options
     * @return \Fabricate\Http\Client\Response|\Fabricate\Http\Client\Promises\LazyPromise
     *
     * @phpstan-return (TAsync is false ? \Fabricate\Http\Client\Response : \Fabricate\Http\Client\Promises\LazyPromise)
     *
     * @throws \Exception
     * @throws \Fabricate\Http\Client\ConnectionException
     */
    public function send(string $method, string $url, array $options = [])
    {
        ...
        return retry($this->tries ?? 1, function ($attempt) use ($method, $url, $options, &$shouldRetry) {
            ...
            $result = ! is_null($shouldRetry) ? $shouldRetry : ($this->retryWhenCallback ? call_user_func($this->retryWhenCallback, $exception, $this, $this->request?->toPsrRequest()->getMethod()) : true);
     ...
     *
     * @param  \Fabricate\Http\Client\Response|\Throwable  $response
     * @param  string  $method
     * @param  string  $url
     * @param  array  $options
     * @param  int  $attempt
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $requestData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $requestData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($requestData) && $method === 'GET' && $urlString->contains('?')) {
            $requestData = (string) $urlString->after('?');
        }

        if (is_string($requestData)) {
            parse_str($requestData, $parsedData);

            $requestData = is_array($parsedData) ? $parsedData : [];
        }
     ...
     * @return \Fabricate\Http\Client\Response
     */
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))
                            ->withData($options['fabricate_data'])
     ...
     * @return \Closure
     *
     * @throws \Fabricate\Http\Client\Exceptions\StrayRequestException
     */
    ...
    {
        return tap(new Response($response), function (Response $fabricateResponse) {
            if (is_null($this->truncateExceptionsAt)) {
                return;
            }

            $this->truncateExceptionsAt === false
                ? $fabricateResponse->dontTruncateExceptions()
                : $fabricateResponse->truncateExceptionsAt($this->truncateExceptionsAt);
        });
    }
     ...
     * @return void
     *
     * @throws \Fabricate\Http\Client\RequestException
     * @throws \Fabricate\Http\Client\ConnectionException
     */
