 ...
 * @method static \Fabricate\Filesystem\FilesystemManager setApplication(\Fabricate\Contracts\Core\Program $app)
 *
...
class Storage extends MagicAlias
{
    ...
    {
        $root = self::getRootPath($disk = enum_value($disk) ?: static::$program['config']->get('filesystems.default'));
    ...
    {
        $disk = enum_value($disk) ?: static::$program['config']->get('filesystems.default');
    ...
    {
        $originalConfig = static::$program['config']["filesystems.disks.{$disk}"] ?? [];
