...
use Fabricate\Core\Events\VendorTagPublished;
...
class VendorPublishCommand extends Command
{
     ...
     *
     * @var \Fabricate\NutsAndBolts\Carbon|null
     */
    ...
    protected function publishTag($tag)
    {
        ...
        if ($publishing === false) {
            ...
            $this->scrapyard_io['events']->dispatch(new VendorTagPublished($tag, $pathsToPublish));
    ...
    protected function publishDirectory($from, $to)
    {
        foreach ($this->files->allFiles($from) as $file) {
            ...
            if (
                (! $this->option('existing') && (! $this->files->exists($target) || $this->option('force')))
                || ($this->option('existing') && $this->files->exists($target))
            ) {
                $target = $this->ensureMigrationNameIsUpToDate($from, $target);
