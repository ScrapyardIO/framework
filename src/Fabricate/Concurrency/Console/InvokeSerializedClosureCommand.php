...
namespace Fabricate\Concurrency\Console;
...
use function Fabricate\Core\Helpers\report;
    ...
    public function handle()
    {
        try {
            $this->output->write(json_encode([
                'successful' => true,
                'result' => serialize($this->getScrapyardIO()->call(match (true) {
                    ! is_null($this->argument('code')) => unserialize($this->argument('code')),
                    isset($_SERVER['SCRAPYARD_IO_INVOKABLE_CLOSURE']) => unserialize(
                        base64_decode($_SERVER['SCRAPYARD_IO_INVOKABLE_CLOSURE'])
                    ),
                    default => fn () => null,
                })),
            ]));
