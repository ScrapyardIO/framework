...
use Fabricate\Database\Events\DatabaseBusy;
...
class MonitorCommand extends DatabaseInspectionCommand
{
     ...
     *
     * @param  \Fabricate\Database\ConnectionResolverInterface  $connection
    ...
    protected function parseDatabases($databases)
    {
        return (new Collection(explode(',', $databases)))->map(function ($database) {
            if (! $database) {
                $database = $this->scrapyard_io['config']['database.default'];
            }
     ...
     *
     * @param  \Fabricate\NutsAndBolts\Collection  $databases
     * @return void
     */
