...
use BareMetal\Circuits\Attributes\SensorSkill;
 ...
 */
#[SensorSkill(AccelerationMeasurable::class)]
class Accelerometer extends SensorComponent implements AccelerometerContract
{
    ...
    public function __construct(
        ...
        parent::__construct($sensor_service);
    }
     ...
     */
    public function sensor(string $sensor = 'default'): static
    {
        if($this->sensor_service->enabled('accelerometers'))
        {
            $config = $this->sensor_service->sensorConfig($sensor, 'accelerometers');

            if($config)
            {
                $this->buildSensor($config);

                return $this;
            }

            throw SensorException::sensorNotRegistered('Accelerometers', $sensor);
        }

        throw SensorException::sensorTypeNotEnabled('Accelerometers');
    }

    protected function buildSensor(array $config): void
    {
        $this->sensor_service->addToActiveSensors($config, $this);
    }
